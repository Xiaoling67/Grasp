...
struct ExportModalView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                HStack {
                    Text("Export Lecture").font(.inter(size: 15, weight: .semibold)).foregroundColor(Color(hex: "0A0A0A"))
                    Spacer()
                    Button("×") { vm.showExportModal = false }.buttonStyle(.plain).font(.inter(size: 14)).foregroundColor(Color(hex: "C0C0C0"))
                }.padding(.horizontal, 20).padding(.top, 16)

                VStack(alignment: .leading, spacing: 18) {
                    if vm.pastLectures.isEmpty {
                        Text("No lectures to export yet.").font(.inter(size: 13)).foregroundColor(Color(hex: "C0C0C0"))
                    } else {
                        VStack(alignment: .leading, spacing: 6) {
                            Text("SELECT LECTURE").font(.inter(size: 11, weight: .semibold)).foregroundColor(Color(hex: "C0C0C0")).tracking(0.3)
                        ...
                        VStack(alignment: .leading, spacing: 8) {
                            Text("INCLUDE IN EXPORT").font(.inter(size: 11, weight: .semibold)).foregroundColor(Color(hex: "C0C0C0")).tracking(0.3)
                            Toggle("Full Transcript", isOn: $opts.t).font(.inter(size: 13)).foregroundColor(Color(hex: "0A0A0A"))
                            Toggle("AI Notes", isOn: $opts.n).font(.inter(size: 13)).foregroundColor(Color(hex: "0A0A0A"))
                            Toggle("Domain Knowledge", isOn: $opts.k).font(.inter(size: 13)).foregroundColor(Color(hex: "0A0A0A"))
                            if DatabaseService.shared.getSetting(key: "defaultMode") == "international" {
                                Toggle("Language Saves", isOn: $opts.l).font(.inter(size: 13)).foregroundColor(Color(hex: "0A0A0A"))
                            }
                            Toggle("AI Searches", isOn: $opts.s).font(.inter(size: 13)).foregroundColor(Color(hex: "0A0A0A"))
                        }
                    }
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Close") { vm.showExportModal = false }
                        ...
                        .foregroundColor(Color(hex: "5A5A5A")).background(Color.clear).cornerRadius(980)
                        .overlay(RoundedRectangle(cornerRadius: 980).stroke(Color(hex: "E8E8E8"), lineWidth: 1)).buttonStyle(.plain)
                    if !vm.pastLectures.isEmpty {
                        Button(status == "loading" ? "Exporting..." : "Export as .rtf") { handle() }
                            .font(.inter(size: 11, weight: .medium)).foregroundColor(.white)
                            .padding(.horizontal, 12).padding(.vertical, 5)
                            .background(Color(hex: "1A5FD4")).cornerRadius(980).buttonStyle(.plain)
