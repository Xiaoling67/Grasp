            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 1) {
                    ...
                    Button(action: { vm.pastExpanded.toggle() }) {
                        HStack { Text("Past Lectures").font(.inter(size: 13, weight: .medium)).foregroundColor(Color.nearBlack); Spacer()
                        ...
                        VStack(spacing: 1) {
                            if vm.pastLectures.isEmpty { Text("No lectures yet").font(.inter(size: 11)).foregroundColor(Color.mutedGray).padding(EdgeInsets(top: 4, leading: 10, bottom: 4, trailing: 10)).frame(maxWidth: .infinity, alignment: .leading) }
                                ...
                                Button(action: { vm.openPastLecture(id: l.id, name: l.name) }) {
                                    Text(l.name ?? "Untitled Lecture").font(.inter(size: 12)).lineLimit(1).foregroundColor(Color.mediumGray).padding(EdgeInsets(top: 4, leading: 10, bottom: 4, trailing: 10)).frame(maxWidth: .infinity, alignment: .leading).cornerRadius(4)
                                }.buttonStyle(.plain)
                            }}
                        }.padding(.leading, 10)
                    }
                ...
                VStack(spacing: 1) {
                    Button(action: { vm.showExportModal = true }) { Text("Export").font(.inter(size: 13)).foregroundColor(Color.mediumGray).padding(EdgeInsets(top: 6, leading: 10, bottom: 6, trailing: 10)).frame(maxWidth: .infinity, alignment: .leading) }.buttonStyle(.plain)
                    NavItem(label: "Settings", page: .settings)
                }.padding(.horizontal, 8)
            }
        }.frame(width: 200).background(Color.fillTertiary)
    ...
    var body: some View {
        Button(action: { vm.activeTabId = nil; vm.page = page }) {
            Text(label).font(.inter(size: 13)).fontWeight(a ? .medium : .regular)
                .foregroundColor(a ? Color.aiNewBorder : Color.mediumGray)
                .padding(EdgeInsets(top: 6, leading: 10, bottom: 6, trailing: 10)).frame(maxWidth: .infinity, alignment: .leading)
                .background(a ? Color.aiNewBorder.opacity(0.08) : Color.clear).cornerRadius(4)
        }.buttonStyle(.plain)
    }
}
