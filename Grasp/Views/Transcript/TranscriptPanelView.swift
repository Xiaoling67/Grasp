                    ...
                    LazyVStack(alignment: .leading, spacing: 0) {
                        if vm.liveBlocks.isEmpty && vm.interimText.isEmpty {
                            ...
                            ForEach(vm.liveBlocks) { b in
                                BlockView(block: b, isActive: b.id == vm.activeBlockId,
                                    onTap: { popup = (b.textEn, b.blockIndex, 200, 0) }).id(b.id)
                            }
...
struct BlockView: View {
    @EnvironmentObject var vm: AppViewModel; let block: LiveBlock; let isActive: Bool; let onTap: () -> Void
    var body: some View {
